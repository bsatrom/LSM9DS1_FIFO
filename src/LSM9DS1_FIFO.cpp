...
void LSM9DS1_FIFO::initI2C(TwoWire *wireBus, int32_t sensorID)
{
    _i2c = true;
    _wire = wireBus;
    ...
    _accelSensor = Sensor(this, &LSM9DS1_FIFO::readAccel, &LSM9DS1_FIFO::getAccelEvent, &LSM9DS1_FIFO::getAccelSensor);
    _magSensor = Sensor(this, &LSM9DS1_FIFO::readMag, &LSM9DS1_FIFO::getMagEvent, &LSM9DS1_FIFO::getMagSensor);
    _gyroSensor = Sensor(this, &LSM9DS1_FIFO::readGyro, &LSM9DS1_FIFO::getGyroEvent, &LSM9DS1_FIFO::getGyroSensor);
    _tempSensor = Sensor(this, &LSM9DS1_FIFO::readTemp, &LSM9DS1_FIFO::getTempEvent, &LSM9DS1_FIFO::getTempSensor);
}

// default
LSM9DS1_FIFO::LSM9DS1_FIFO(int32_t sensorID)
{
    initI2C(&Wire, sensorID);
}

LSM9DS1_FIFO::LSM9DS1_FIFO(TwoWire *wireBus, int32_t sensorID)
{
    initI2C(wireBus, sensorID);
}

LSM9DS1_FIFO::LSM9DS1_FIFO(int8_t xgcs, int8_t mcs, int32_t sensorID)
{
    _i2c = false;
    // hardware SPI!
    _csm = mcs;
...
LSM9DS1_FIFO::LSM9DS1_FIFO(int8_t sclk, int8_t smiso, int8_t smosi, int8_t xgcs, int8_t mcs, int32_t sensorID)
{
    _i2c = false;
    // software SPI!
    _csm = mcs;
...
bool LSM9DS1_FIFO::begin()
{
  ...
  */
    ...
    write8(XGTYPE, LSM9DS1_REGISTER_CTRL_REG9, 0x02);
    // Set FIFO continuous mode
    write8(XGTYPE, LSM9DS1_REGISTER_CTRL_FIFO, 0xC0);

    // Enable the accelerometer
    write8(XGTYPE, LSM9DS1_REGISTER_CTRL_REG5_XL, 0x38); // enable X Y and Z axis
    write8(XGTYPE, LSM9DS1_REGISTER_CTRL_REG1_G, 0x78);  // 119 Hz, 2000 dps, 16 Hz BW
    write8(XGTYPE, LSM9DS1_REGISTER_CTRL_REG6_XL, 0x70); // 119 Hz, 4G
...
int LSM9DS1_FIFO::accelerationAvailable()
{
    // Read FIFO_SRC. If any of the rightmost 8 bits have a value, there is data
    if (read8(XGTYPE, 0x2F) & 63)
    {
        return 1;
    }
...
void LSM9DS1_FIFO::setupAccel(lsm9ds1AccelRange_t range)
{
...
void LSM9DS1_FIFO::setupMag(lsm9ds1MagGain_t gain)
{
...
void LSM9DS1_FIFO::setupGyro(lsm9ds1GyroScale_t scale)
{
...
*/
/**************************************************************************/
bool LSM9DS1_FIFO::getEvent(sensors_event_t *accelEvent,
...
*/
/**************************************************************************/
void LSM9DS1_FIFO::getSensor(sensor_t *accel, sensor_t *mag,
...
/***************************************************************************
 PRIVATE FUNCTIONS
 ***************************************************************************/
void LSM9DS1_FIFO::write8(boolean type, byte reg, byte value)
{
    byte address, _cs;
...
byte LSM9DS1_FIFO::read8(boolean type, byte reg)
{
    uint8_t value;
...
byte LSM9DS1_FIFO::readBuffer(boolean type, byte reg, byte len, uint8_t *buffer)
{
    byte address, _cs;
...
uint8_t LSM9DS1_FIFO::spixfer(uint8_t data)
{
...
void LSM9DS1_FIFO::getAccelEvent(sensors_event_t *event, uint32_t timestamp)
{
...
void LSM9DS1_FIFO::getMagEvent(sensors_event_t *event, uint32_t timestamp)
{
    memset(event, 0, sizeof(sensors_event_t));
    event->version = sizeof(sensors_event_t);
...
void LSM9DS1_FIFO::getGyroEvent(sensors_event_t *event, uint32_t timestamp)
{
    memset(event, 0, sizeof(sensors_event_t));
    event->version = sizeof(sensors_event_t);
    event->sensor_id = _lsm9dso_sensorid_accel;
...
void LSM9DS1_FIFO::getTempEvent(sensors_event_t *event, uint32_t timestamp)
{
    memset(event, 0, sizeof(sensors_event_t));
    event->version = sizeof(sensors_event_t);
...
void LSM9DS1_FIFO::getAccelSensor(sensor_t *sensor)
{
...
void LSM9DS1_FIFO::getMagSensor(sensor_t *sensor)
{
    memset(sensor, 0, sizeof(sensor_t));
...
void LSM9DS1_FIFO::getGyroSensor(sensor_t *sensor)
{
    memset(sensor, 0, sizeof(sensor_t));
...
void LSM9DS1_FIFO::getTempSensor(sensor_t *sensor)
{
    memset(sensor, 0, sizeof(sensor_t));
